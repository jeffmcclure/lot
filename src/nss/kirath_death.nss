...
void main()
{
    ...
    CreateItemOnObject("humanbones001",OBJECT_SELF,1);
    object ocontroldeathblock = GetObjectByTag("controldeathblock");
    SetLocalInt(ocontroldeathblock, "kirath died" , 1);
    int Get_Beohram_HitDice_at_Death = GetHitDice(OBJECT_SELF);
    SetLocalInt(ocontroldeathblock, "Save Kirath Hit Dice level for rez", Get_Beohram_HitDice_at_Death);

    //SetLocalInt(ocontroldeathblock, "SetIsDestroyable is set for Kirath, TRUE, TRUE, TRUE", 0);
}
