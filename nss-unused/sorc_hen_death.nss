...
const int HENCH_SALVATION  = 0;

void main()
{
    //Henchman Death Option: Set HENCH_BLEED = 1 for henchman bleeding
    //                       Set HENCH_BLEED = 0 for default BIOE death
    int HENCH_BLEED = 1;
...
void BringBack()
{
    ...
    if (GetIsObjectValid(oWay) == TRUE)
    {
        ...
        if (GetTag(GetArea(OBJECT_SELF)) == "HELL4")
        {
            DelayCommand(0.2, JumpToObject(GetObjectByTag("WP_HELL4"), FALSE));
        }
        else
